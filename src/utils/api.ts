...
export const login = async (data: ILoginPayload): Promise<void> => {
  try {
    const response: AxiosResponse<IUserWithTokens> = await api.post(
      API_ROUTES.LOG_IN,
      data
    );

    localStoreHelper.setLocalStorageItem(
...
export const createOrder = async (
  data: IOrderPayload
): Promise<IProduct & { orderId: string }> => {
  const currentUser = localStoreHelper.getLocalStorageItem<IUser>(
    LOCAL_STORAGE_KEYS.user
  );

  try {
    const response: AxiosResponse<IProduct & { orderId: string }> =
      await api.post(API_ROUTES.ORDER(data.productId?.toString()), {
        ...data,
        user: currentUser,
      });
...
export const getOrders = async (currentUser: IUser): Promise<IFullOrder[]> => {
  try {
    const response: AxiosResponse<IFullOrder[]> = await api.get(
      `${API_ROUTES.ORDERS}/${currentUser?.id}`
    );
